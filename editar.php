...
$serial = test_input( $_REQUEST['serial'] ?? null );

$juego= getJuegobyID($conexion,$serial-10000);
...
$nombre = test_input( $_POST['nombre'] ?? $juego['nombre'] );
$precio = test_input( $_POST['precio'] ?? $juego['precio'] );
$descripcion = test_input( $_POST['descripcion'] ?? $juego['descripcion'] );
$calificacion = test_input($_POST['calificacion']?? $juego['calificacion']);
$desarrollador = test_input($_POST['desarrollador']?? $juego['desarrollador']);
$lanzamiento = test_input($_POST['lanzamiento']?? $juego['lanzamiento']);
$trailer = test_input($_POST['trailer']?? $juego['trailer']);
$etiquetas = test_input($_POST['etiquetas']?? $juego['etiquetas']);
...
if( isset($_POST['añadir']) )
{
    ...
    if( count($errores) == 0 )
    {
        ...
        if( $portada['error'] == '0' ){
            $imagen_path = 'img/portadas/por_' . time() . $portada['name'];
            move_uploaded_file( $portada['tmp_name'], $imagen_path );
            unlink($juego['portada']);
        }else{
            $imagen_path = $juego['portada'];
        }


        updateJuego($conexion,$serial-10000, array(
            'nombre' => $nombre,
...
<head>
    <title>Editar Producto | Vapor Gaming</title>
    <?php
